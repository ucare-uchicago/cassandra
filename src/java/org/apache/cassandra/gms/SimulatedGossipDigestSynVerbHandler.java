...
//        }
        ...
        Gossiper.notifyFailureDetectorStatic(stub, gDigestList);
...
//                      new GossipDigestAck(deltaGossipDigestList, deltaEpStateMap), GossipDigestAck.serializer);
        MessageIn<GossipDigestAck> ackMessageIn = MessageIn.create(to, new GossipDigestAck(deltaGossipDigestList, deltaEpStateMap), 
                emptyMap, MessagingService.Verb.GOSSIP_DIGEST_ACK, MessagingService.VERSION_12);
        ackMessageIn.setTo(from);
        GossipSimulator.sendMessage(ackMessageIn, from);
        if (logger.isTraceEnabled())
            logger.trace("Sending a GossipDigestAckMessage to {}", from);
//        MessagingService.instance().sendOneWay(gDigestAckMessage, from);
//        GossipSimulator.sendMessage(msg, address);
    }
